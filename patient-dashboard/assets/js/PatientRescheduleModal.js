 ...
 *
 * Usage:
 *   const resched = new PatientRescheduleModal({ state: PatientState, onRescheduled: () => loadPatientAppointments() });
 *   resched.init();
 */
/* global Modal, showToast */
window.PatientRescheduleModal = class PatientRescheduleModal {
  constructor ({ state, appointmentsEndpoint = '../backend/api/patients/appointments.php', onRescheduled = null } = {}) {
    this.state          = state;
