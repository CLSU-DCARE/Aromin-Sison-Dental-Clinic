 ...
 * Staff accounts remain administrator-created.
 */
...
$firstName =
    ...
    && is_string($input['first_name'])
        ? trim($input['first_name'])
        : '';

$lastName =
    isset($input['last_name'])
    && is_string($input['last_name'])
...
$contactNumber =
    isset($input['contact_number'])
    && is_string($input['contact_number'])
...
try {
    ...
    /*
     * Every public signup receives a new patient
     * record. It will not automatically inherit an
     * old patient's appointments based on email.
     */
